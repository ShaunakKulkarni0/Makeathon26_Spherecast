...
from src.crawling.crawler.capsuline import run_capsuline_deep_crawl

logger = logging.getLogger(__name__)

CACHE_FILE = "data/capsuline_query_cache.json"

def get_synonyms_from_gpt(group_name: str, api_key: str) -> list:
    """Uses GPT-4o-mini to generate synonyms for capsuline catalog."""
    if not api_key:
...
def process_targets(csv_path: str):
    ...
    # Filter for Capsuline
    targets_df = df[df['supplier_names'].str.contains('capsuline', case=False, na=False)]
    unique_groups = targets_df['group_name'].str.lower().str.strip().unique().tolist()
    
    logger.info(f"Identified {len(unique_groups)} unique product groups related to Capsuline.")
    ...
    # Load Env
    load_dotenv()
    # It might be OPENAI_API_KEY, let's grab it directly
    ...
    for i, group in enumerate(unique_groups):
        ...
        results = search_capsuline(group)
        used_synonym = None
    ...
    if unique_urls:
        ...
        run_capsuline_deep_crawl(
            search_queries=[],
            output_json_path="data/capsuline_crawler_output.json",
